...
app.delete('/api/admin/user/:userId', async (req, res) => {
    try {
        await db.execute('DELETE FROM Users WHERE UserId = ? AND Role != "ADMIN"', [req.params.userId]);
...
app.post('/api/admin/approve-business', async (req, res) => {
    try {
        ...
        await db.execute('UPDATE Business_Account SET Approval_Status = "APPROVED" WHERE Business_Id = ?', [businessId]);
...
app.post('/api/events/update-seats', async (req, res) => {
    try {
        ...
        const [[event]] = await db.execute('SELECT Total_Seats, Available_Seats FROM Event WHERE Event_Id = ?', [eventId]);
...
app.get('/api/admin/bookings', async (req, res) => {
    try {
        ...
        for (let i = 0; i < bookings.length; i++) {
            const [tickets] = await db.execute('SELECT * FROM Ticket WHERE Booking_Id = ?', [bookings[i].Booking_Id]);
...
app.post('/api/admin/repair-seats', async (req, res) => {
    try {
        ...
        for (const ev of events) {
            ...
            await db.execute('UPDATE Event SET Available_Seats = ? WHERE Event_Id = ?', [correctAvailable, ev.Event_Id]);
            fixed++;
        }
...
app.get('/api/business/bookings/:userId', async (req, res) => {
    try {
        ...
        for (let i = 0; i < bookings.length; i++) {
            const [tickets] = await db.execute('SELECT Ticket_Id, Seat_Number FROM Ticket WHERE Booking_Id = ?', [bookings[i].Booking_Id]);
            bookings[i].tickets = tickets;
            const [payments] = await db.execute('SELECT Payment_Id, Amount, Payment_Status, Transaction_Id FROM Payment WHERE Booking_Id = ? LIMIT 1', [bookings[i].Booking_Id]);
...
app.get('/api/tickets/:bookingId', async (req, res) => {
    try {
        const [rows] = await db.execute('SELECT * FROM Ticket WHERE Booking_Id = ?', [req.params.bookingId]);
...
app.post('/api/waitlist/join', async (req, res) => {
    try {
        ...
        const [bl] = await db.execute('SELECT * FROM Blacklist WHERE UserId = ?', [userId]);
...
app.post('/api/tickets/transfer', async (req, res) => {
    try {
        ...
        const [[existing]] = await db.execute('SELECT Transfer_Id FROM Ticket_Transfer WHERE Ticket_Id = ? AND Status = "PENDING"', [ticketId]);
        if (existing) return res.status(400).json({ success: false, error: 'A pending transfer already exists for this ticket.' });
        const [[toUser]] = await db.execute('SELECT UserId, Name FROM Users WHERE Email = ?', [toEmail]);
        ...
        const [bl] = await db.execute('SELECT * FROM Blacklist WHERE UserId = ?', [toUser.UserId]);
...
app.get('/api/admin/analytics', async (req, res) => {
    try {
        ...
        const [sellSpeed] = await db.execute(
...
app.post('/api/events/duplicate/:eventId', async (req, res) => {
    try {
        const [[ev]] = await db.execute('SELECT * FROM Event WHERE Event_Id = ?', [req.params.eventId]);
...
app.get('/api/rewards/:userId', async (req, res) => {
    try {
        let [rows] = await db.execute('SELECT * FROM User_Rewards WHERE UserId = ?', [req.params.userId]);
        if (rows.length === 0) {
            const id = Math.floor(Math.random() * 1000000);
            await db.execute('INSERT INTO User_Rewards (Reward_Id, UserId) VALUES (?, ?)', [id, req.params.userId]);
...
app.get('/api/messages/contacts/:userId', async (req, res) => {
    try {
        const uid = req.params.userId;
        const [[currentUser]] = await db.execute('SELECT Role FROM Users WHERE UserId = ?', [uid]);
        let query;
